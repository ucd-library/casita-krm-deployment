...
POSTGIS_IMAGE_NAME=$CONTAINER_REG_ORG/krm-postgis
RING_BUFFER_IMAGE_NAME=$CONTAINER_REG_ORG/krm-blocks-ring-buffer
...
EXTERNAL_TOPICS_IMAGE_NAME=$CONTAINER_REG_ORG/external-topics-worker
ZOOKEEPER_IMAGE_NAME=zookeeper
KAFKA_IMAGE_NAME=bitnami/kafka
MONGO_IMAGE_NAME=mongo
RABBITMQ_IMAGE_NAME=rabbitmq
OPEN_KAFKA_IMAGE_NAME=gcr.io/ucdlib-pubreg/open-kafka
...
ALL_DOCKER_BUILD_IMAGES=( \
  ...
  $POSTGIS_IMAGE_NAME $RING_BUFFER_IMAGE_NAME $EXTERNAL_TOPICS_IMAGE_NAME
)
